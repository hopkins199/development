...
function listData(data){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        containerDiv.id = todoId
        const todoInfoContainer = document.createElement('div')
        todoInfoContainer.classList.add('infoContainer')
        containerDiv.appendChild(todoInfoContainer)
        containerDiv.classList.add('conatiner')
        document.getElementById('todo-list').appendChild(containerDiv)
        const imgrr = document.createElement('img')
        imgrr.class = "pics"
        imgrr.src = data[i].imgUrl
       containerDiv.appendChild(imgrr)
        const h1 = document.createElement('h1')
        h1.textContent = data[i].title
        h1.id = "title"
       todoInfoContainer.appendChild(h1)
        const description = document.createElement('p')
        description.id = "description"
        description.textContent = data[i].description
       todoInfoContainer.appendChild(description)
        const price = document.createElement('p')
        price.id = "price"
        price.textContent = 'What will it Cost: ' + data[i].price
       todoInfoContainer.appendChild(price)
        // Delete Button
        const button1 = document.createElement('button')
        button1.innerHTML = "Delete"
        todoInfoContainer.appendChild(button1)
        ...
        const container =todoInfoContainer;
